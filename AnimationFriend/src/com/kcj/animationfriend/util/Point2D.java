...
public class Point2D {
	...
	public static double hypotenuse(RectF rect) {
		return Math.sqrt(Math.pow(rect.right - rect.left, 2.0D)
				+ Math.pow(rect.bottom - rect.top, 2.0D));
	}
	...
	public static void rotateAroundBy(PointF position, PointF center,
			float angle) {
		double angleInRadians = angle * 0.0174532925199433D;
		double cosTheta = Math.cos(angleInRadians);
		double sinTheta = Math.sin(angleInRadians);

		position.x = ((float) (cosTheta * (position.x - center.x) - sinTheta
				* (position.y - center.y) + center.x));
		position.y = ((float) (sinTheta * (position.x - center.x) + cosTheta
				* (position.y - center.y) + center.y));
	}
	...
	public static PointF intersection(PointF[] a, PointF[] b) {
		...
		return new PointF(((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2)
				* (x3 * y4 - y3 * x4))
