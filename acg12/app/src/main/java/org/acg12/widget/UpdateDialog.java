...
public class UpdateDialog extends BaseDialog implements View.OnClickListener{
	...
	public void onClick(View v) {
		...
		if(id == R.id.btn_checkable){
			...
			DaoBaseImpl.getInstance().saveUpdate(update);
		}
	}
	...
	public void show() {
		if(update.getDialogStatus() == 0){
			...
			setCancelable(true);
			setButton1("取消", new OnClickListener() {
				@Override
...
//					DownloadService.getDownloadManager().startDownloadApp(mContext ,update, true, false);
	...
	public void initNotify(){
		...
		mRemoteViews = new RemoteViews(ACGApplication.getInstance().getPackageName(), R.layout.include_notify_progress);
