...
import org.acg12.listener.ISkinLoader;
...
public class SkinManager implements ISkinLoader {
	...
	public ColorStateList convertToColorStateList(int resId) {
		Log.i("attr1", "convertToColorStateList");
		...
		String resName = context.getResources().getResourceEntryName(resId);
		Log.i("attr1", "resName = " + resName);
		if (isExtendSkin) {
			Log.i("attr1", "isExtendSkin");
			int trueResId = mResources.getIdentifier(resName, "color", skinPackageName);
			Log.i("attr1", "trueResId = " + trueResId);
			...
			if (trueResId == 0) { // 如果皮肤包没有复写该资源，但是需要判断是否是ColorStateList
				try {
					...
					e.printStackTrace();
					Log.i("","resName = " + resName + " NotFoundException : "+ e.getMessage());
				}
			} else {
				try {
					trueColorList = mResources.getColorStateList(trueResId);
					Log.i("attr1", "trueColorList = " + trueColorList);
