...
import org.acg12.net.HomeRequest;
...
public class HomeRequestImpl implements HomeRequest {
    ...
    public HomeRequestImpl(Context mContext) {
    ...
    public Subscription newsList(User user, String page, final HttpRequestListener<List<News>> httpRequestListener) {
        return RetrofitClient.with(user).newsList(page)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<ResponseBody>() {
                    @Override
                    public void call(ResponseBody response) {
                        ...
                        if (data != null) {
                            ...
                            JSONArray jsonArray = RetrofitClient.getJSONArray(data, "list");
                            for (int i = 0, num = jsonArray.length(); i < num; i++) {
                                JSONObject item = RetrofitClient.getJSONObject(jsonArray, i);
                                News news = new News();
                                news.setNewsId(RetrofitClient.getInt(item, "article_id"));
                                news.setCover(RetrofitClient.getString(item, "pic_url"));
                                news.setTitle(RetrofitClient.getString(item, "title"));
                                news.setIntro(RetrofitClient.getString(item, "intro"));
                                news.setLink(RetrofitClient.getString(item, "page_url"));
                                news.setCreateTime(RetrofitClient.getInt(item, "create_time"));
                                list.add(news);
                            }
    ...
    public Subscription searchAlbum(User user, String key, String page, final HttpRequestListener<List<Album>> httpRequestListener) {
        Subscription subscription = RetrofitClient.with(user).searchAlbum(URLEncoderUtil.encode(key), page)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<ResponseBody>() {
                    @Override
    ...
    public Subscription searchPalette(User user, String key, String page, final HttpRequestListener<List<Palette>> httpRequestListener) {
        Subscription subscription = RetrofitClient.with(user).searchPalette(URLEncoderUtil.encode(key), page)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<ResponseBody>() {
                    @Override
...
////        final HomeApi apiService = RetrofitClient.with(user);
