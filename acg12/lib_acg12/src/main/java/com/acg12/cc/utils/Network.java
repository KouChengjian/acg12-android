...
public class Network {
	...
	public static boolean printNetworkInfo(Context context) {
		...
		if (connectivity != null) {
			...
			Log.e(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
			Log.e(TAG, "getActiveNetworkInfo: " + in);
			NetworkInfo[] info = connectivity.getAllNetworkInfo();
			if (info != null) {
				for (int i = 0; i < info.length; i++) {
					// if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
					Log.e(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
					Log.e(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
					Log.e(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
					Log.e(TAG, "NetworkInfo[" + i + "]: " + info[i]);
					// }
				}
				Log.e(TAG, "\n");
			} else {
				Log.e(TAG, "getAllNetworkInfo is null");
			}
		}
		return false;
	}
	...
	public static String getLocalIPAddress(Context ctx) {
		...
		WifiManager wifimanage = (WifiManager) ctx.getSystemService(ctx.WIFI_SERVICE);// 获取WifiManager
		// 检查wifi是否开启
		...
		try {
			for (Enumeration<NetworkInterface> mEnumeration = NetworkInterface
				...
				for (Enumeration<InetAddress> enumIPAddr = intf
					...
					InetAddress inetAddress = enumIPAddr.nextElement(); // 如果不是回环地址
					if (!inetAddress.isLoopbackAddress()) { // 直接返回本地IP地址
