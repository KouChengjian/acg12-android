...
public class DownloadManger {
    ...
    private Map<String, DownLoadEntity> downloadDataMap = new HashMap<>();//保存任务数据
    ...
    private DownloadManger(Context context) {
        ...
        List<DownLoadEntity> list = DownLoadDao.INSTANCE.queryDownloadList();
    ...
    public void init(String url, String path, String name, int childTaskCount) {
        downloadData = new DownLoadEntity();
     ...
     * @return
     */
    public DownloadManger start(DownLoadEntity downloadData, DownLoadCallback downloadCallback) {
        execute(downloadData, downloadCallback);
        return downloadManager;
    }
     ...
     * @param downloadCallback
     */
    public void setOnDownloadCallback(DownLoadEntity downloadData, DownLoadCallback downloadCallback) {
     ...
     * @return
     */
    public DownLoadEntity getDbData(String name) {
        return DownLoadDao.INSTANCE.queryUrlDownLoad(name);
    }
     ...
     * @return
     */
    public List<DownLoadEntity> getAllDbData() {
        return DownLoadDao.INSTANCE.queryDownloadList();
    }
     ...
     * @return
     */
    public DownLoadEntity getCurrentData(String url) {
     ...
     * 执行下载任务
     */
    private void execute(DownLoadEntity downloadData, DownLoadCallback downloadCallback) {
        //防止同一个任务多次下载
        if (progressHandlerMap.get(downloadData.getName()) != null) {
            return;
        }
